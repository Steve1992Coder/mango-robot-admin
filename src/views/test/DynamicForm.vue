<template>
  <el-form ref="formRef" :model="dynamicValidateForm" label-width="auto" class="demo-dynamic w-full">
    ...
    <el-card v-for="(domain, index) in dynamicValidateForm.domains" :key="domain.key" class="mb-6" shadow="never">
      <template #header>
      ...
      <el-form-item label="域名" :prop="'domains.' + index + '.domain'"
                    :rules="{ required: true, message: '域名不能为空', trigger: 'blur' }">
        <el-input v-model="domain.domain" />
      </el-form-item>

      <el-form-item label="url映射" require>
        <el-row v-for="(urlMap, urlindex) in domain.urlMaps" :key="urlMap.key" :gutter="20" class="mb-6 w-full">
          <el-col :span="10">
            <el-form-item :prop="'domains.' + index + '.urlMaps' + '.' + urlindex + '.url'"
                          :rules="{ required: true, message: '不能为空', trigger: 'blur' }">
              <el-input v-model="urlMap.url" :placeholder="'url映射' + urlindex" />
            </el-form-item>
          </el-col>
          <el-col :span="10">
            <el-form-item :prop="'domains.' + index + '.urlMaps' + '.' + urlindex + '.urlMapName'"
                          :rules="{ required: true, message: '不能为空', trigger: 'blur' }">
        ...
        <el-form-item class="w-full">
          <el-button @click="addUrlMap(index)" class="w-full dashedButton" plain>+新增映射</el-button>
        </el-form-item>
